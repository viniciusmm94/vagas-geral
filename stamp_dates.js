...
// o campo `detectado_em` em cada linha de vagas_merged.json. Vagas novas ganham a
// data de hoje -> a coluna "Detectada em" mostra na hora o que abriu desde a ultima rodada.
const fs = require('fs'), path = require('path');
const DIR = __dirname;
const VF = path.join(DIR, 'vagas_merged.json');
...
const norm = s => String(s || '').normalize('NFD').replace(/[Ì€-Í¯]/g, '').toLowerCase().replace(/[^a-z0-9]+/g, '');
